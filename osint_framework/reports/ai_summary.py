...
class AIReportGenerator:
    ...
    async def generate_summary(self, job_data: Dict[str, Any]) -> str:
        ...
        logger.info(f"Generating AI Summary for target {condensed_data['target']} via Ollama ({self.model})")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logger.error(f"Ollama API Error: {resp.text}")
                    return f"Failed to generate summary: HTTP {resp.status_code}"
            except httpx.ConnectError:
                logger.error("Could not connect to Ollama. Ensure Ollama is running locally on port 11434.")
